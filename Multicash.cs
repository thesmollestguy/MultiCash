...
public class Multicash : BloonsTD6Mod
{
    ...
    private static readonly ModSettingHotkey OpenGUI = new ModSettingHotkey(KeyCode.F10, Il2CppAssets.Scripts.Unity.UI_New.InGame.HotkeyModifier.None)
    {
        displayName = "Multicash Hotkey"
    };

    private static readonly ModSettingHotkey OpenPercentGUI = new ModSettingHotkey(KeyCode.F10, Il2CppAssets.Scripts.Unity.UI_New.InGame.HotkeyModifier.Shift)
    {
        displayName = "Multicash Percentage Hotkey"
    };
    ...
    public override void OnUpdate()
    {
        ...
        if (OpenPercentGUI.JustPressed())
        {
            Action<int> mod = delegate (int s)
            {
                Multiplier = (s / 100.0);
            };
            PopupScreen.instance.ShowSetValuePopup("Set Percentage (Current: " + (Multiplier * 100) + "%)", "Multiply by (%)", mod, 100);

            displayOpen = true;
        }
        else if (OpenGUI.JustPressed())
        {
            Action<int> mod = delegate (int s)
            {
                Multiplier = s;
            ...
            PopupScreen.instance.ShowSetValuePopup("Set Multiplier (Current: " + (Multiplier*1) + "x)", "Multiply by", mod, 1);
